...
from ..axis_partition import ArrowOnRayColumnPartition, ArrowOnRayRowPartition
from ..remote_partition import ArrowOnRayRemotePartition


class ArrowOnRayBlockPartitions(BaseBlockPartitions):
    """This method implements the interface in `BaseBlockPartitions`."""

    # This object uses RayRemotePartition objects as the underlying store.
    _partition_class = ArrowOnRayRemotePartition
    ...
    def column_partitions(self):
        ...
        return [ArrowOnRayColumnPartition(col) for col in self.partitions.T]

    @property
    def row_partitions(self):
        """A list of `PandasOnRayRowPartition` objects."""
        return [ArrowOnRayRowPartition(row) for row in self.partitions]
