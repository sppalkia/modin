...
from ...partitioning.remote_partition import ArrowOnRayRemotePartition
from .utils import split_result_of_axis_func_pandas


class ArrowOnRayAxisPartition(BaseAxisPartition):
...
class ArrowOnRayColumnPartition(ArrowOnRayAxisPartition):
...
class ArrowOnRayRowPartition(ArrowOnRayAxisPartition):
...
def deploy_ray_axis_func(axis, func, num_splits, kwargs, *partitions):
    ...
    """
    partitions = [part.to_pandas() for part in partitions]
    ...
    if num_splits != len(partitions) or isinstance(result, pandas.Series):
        ...
        if axis == 0:
            ...
            if sum(lengths) != len(result.columns):
                lengths = None
    splits = split_result_of_axis_func_pandas(axis, num_splits, result, lengths)
    return [pyarrow.Table.from_pandas(split) for split in splits]
...
def deploy_ray_func_between_two_axis_partitions(
    ...
    splits = split_result_of_axis_func_pandas(axis, num_splits, result)
    return [pyarrow.Table.from_pandas(split) for split in splits]
